"""
URL configuration for Prueba project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/4.2/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from . import views
from django.conf import settings
from django.conf.urls.static import static

urlpatterns = [
    path('admin/', admin.site.urls),
    path('', views.inicio, name='inicio'),
    path('tablero/', views.tablero, name='tablero'),
    path('register', views.register_user, name='register_user'), 
    path('login',views.login_user, name='login'),
    path('logout',views.logout_user, name='logout'),
    path('perfil/', views.perfil, name='perfil'),
    path('guardar-horario/', views.guardar_horario, name='guardar_horario'),
    path('guardar-ramos/', views.guardar_ramos, name='guardar_ramos'),
    path('perfil/<int:user_id>/', views.perfil_publico, name='perfil_publico'),
    path('guardar-datos/', views.guardar_datos, name='guardar_datos'),
    path('agendar-hora/', views.agendar_hora, name='agendar_hora'),
    path('gestionar-reserva/', views.gestionar_reserva, name='gestionar_reserva'),
    path('notificaciones/', views.notificaciones, name='notificaciones'),
    path('notificaciones/count/', views.notificaciones_count, name='notificaciones_count'),
    path('mis-profesores/', views.mis_profesores, name='mis_profesores'),
    path('admin-usuarios/', views.admin_usuarios, name='admin_usuarios'),
    path('subir-foto/', views.subir_foto, name='subir_foto'),
    path('historial/', views.historial, name='historial'),
    path('resenar/<int:user_id>/', views.dejar_resena, name='dejar_resena'),
    path('perfil/semana-est/', views.semana_estudiante, name='semana_estudiante'),
    path('perfil/semana-prof/', views.semana_profesor, name='semana_profesor'),
    path('perfil/<int:user_id>/semana/', views.semana_publica, name='semana_publica'),
    path('recuperar-contrasena/', views.recuperar_contrasena, name='recuperar_contrasena'),
    path('recuperar-contrasena/<uid64>/<token>/', views.recuperar_contrasena_confirmar, name='recuperar_contrasena_confirmar'),
] + static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)

